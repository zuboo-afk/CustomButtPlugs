...
def main():
  ...
  if not zip:
    installer = "\\build-win\\installer\\SubPlug Installer.exe"
    
    if demo:
      installer = "\\build-win\\installer\\SubPlug Demo Installer.exe"
    
    files = [
      projectpath + installer,
      projectpath + "\\installer\\changelog.txt",
      projectpath + "\\installer\\known-issues.txt",
      projectpath + "\\manual\\SubPlug manual.pdf" 
    ]
  else:
    files = [
      projectpath + "\\build-win\\SubPlug.vst3\\Contents\\x86_64-win\\SubPlug.vst3",
      projectpath + "\\build-win\\SubPlug_x64.exe",
      projectpath + "\\build-win\\SubPlug.clap"
    ]
  ...
  files = [
    projectpath + "\\build-win\\pdbs\\SubPlug-vst3_x64.pdb",
    projectpath + "\\build-win\\pdbs\\SubPlug-app_x64.pdb",
    projectpath + "\\build-win\\pdbs\\SubPlug-clap_x64.pdb"
  ]
